...
bool EthernetConnection_::setup() {
    delay(5000);
    Logger.info("Setting up Ethernet (W5500)...");
    ...
    // Инициализация CS пина
    pinMode(W5500_CS_PIN, OUTPUT);
    digitalWrite(W5500_CS_PIN, HIGH); // деактивирован

    // Инициализация Ethernet библиотеки с указанием пина CS
    Ethernet.init(W5500_CS_PIN);
    
    // Начало работы Ethernet с использованием статической конфигурации.
    // Предполагается, что переменные mac, staticIP, gateway, subnet, primaryDNS определены в Settings.h
    Logger.info("Starting Ethernet with static IP configuration...");
    // int ethStatus = Ethernet.begin(mac, staticIP, primaryDNS, gateway, subnet);
    Ethernet.begin(mac, staticIP, primaryDNS, gateway, subnet);
    // if (ethStatus == 0) {
    //     Logger.warn("Ethernet initialization failed");
    //     return false;
    // } else {
...
void EthernetConnection_::_handleInterrupt() {
    // Обработчик прерывания можно использовать для обновления статуса соединения,
    // например, переинициализировать соединение при его потере.
}
