...
public:
    ...
    Stepper(PCF8574* i2c_address, uint8_t pin1 = 255, uint8_t pin2 = 255, uint8_t pin3 = 255, uint8_t pin4 = 255, uint8_t pin5 = 255) {
        if (_TYPE == STEPPER_PINS) {
            if (_DRV == STEPPER_I2C) {
				...
				configurePin_i2c(0, pin1);
                configurePin_i2c(1, pin2);
                if (pin3 != 255) {
                    _enPin = pin3;
                    _i2c_address->pinMode(_enPin, OUTPUT);
                }
			}
			else if (_DRV == STEPPER2WIRE) {
                configurePin(0, pin1);
                configurePin(1, pin2);
                if (pin3 != 255) {
                    _enPin = pin3;
    ...
    void step() {
        pos += dir;
        if (_DRV == STEPPER2WIRE || _DRV == STEPPER_I2C) {     // ~4 + DRIVER_STEP_TIME us
            stepDir();
        } else {                        // ~5.5 us
    ...
    void reverse(bool val) {
        if (_DRV == STEPPER2WIRE || _DRV == STEPPER_I2C) {
    ...
    void disable() {
        if (_TYPE == STEPPER_PINS) {
            ...
            if (_enPin != 255) {
				if (_DRV != STEPPER_I2C) digitalWrite(_enPin, !_enDir);
				else _i2c_address->digitalWrite(_enPin, !_enDir);
			}
        } else {
            if (*_power) _power(0);
...
private:        
	...
	void setPin_i2c(int num, bool state) {
		_i2c_address->digitalWrite(_pins[num], !state); ///!state); // в мультиплексере почему-то когда ставишь LOW - есть напряжение, HIGH-нет
		//Serial.println("Set pin" + String(_pins[num]) + " to " + String(!state));
	}
    ...
    void stepDir() {
        if (_TYPE == STEPPER_PINS) {
            if (_DRV == STEPPER_I2C){
				if (_pdir != dir) {
					_pdir = dir;
					setPin_i2c(1, (dir > 0) ^ _globDir);	// DIR
				}
				setPin_i2c(0, 1);	// step HIGH
				if (DRIVER_STEP_TIME > 0) delayMicroseconds(DRIVER_STEP_TIME);
				setPin_i2c(0, 0);	// step LOW
			} else {
				if (_pdir != dir) {
					_pdir = dir;
